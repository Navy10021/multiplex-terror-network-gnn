...
def load_multiplex(manifest_path: str):
    ...
    Supports:
      ...
      - v3: same as v2, but edges may include aggregated statistics
    """
...
def _aggregate_from_events(events: List[Dict[str, Any]]) -> Tuple[Dict[Tuple[int, int], Dict[str, float]], Dict[Tuple[int, int], Dict[str, float]], Dict[Tuple[int, int], Dict[str, float]]]:
...
def build_pyg_data(
    ...
    def _edge_weight(layer_name: str, u: int, v: int, row: Optional[pd.Series], directed: bool) -> float:
        ...
        if layer_name == "communication":
            if row is not None:
                for key in ["comm_count", "num_events"]:
                    if key in row and row.get(key) is not None:
                        return _safe_log1p(float(row.get(key)))
                if "comm_duration_sum" in row and row.get("comm_duration_sum") is not None:
        ...
        if layer_name == "ideology":
            if row is not None and "similarity" in row and row.get("similarity") is not None:
                try:
                    return float(row.get("similarity"))
                except Exception:
                    return 0.0
            # fallback: if node ideology exists, similarity can be computed outside,
            # but we keep 0.0 here to avoid any accidental mismatch.
            return 0.0

        return 0.0

    def _add_edge(u_id: int, v_id: int, ltype: int, w: float):
    ...
    for lname in layer_order:
        ...
        for _, row in df_layer.iterrows():
            ...
            _add_edge(u, v, ltype, w)

            if symmetrize_undirected and not directed:
                _add_edge(v, u, ltype, w)
