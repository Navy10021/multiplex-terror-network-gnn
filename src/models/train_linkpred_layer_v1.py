# src/models/train_linkpred_layer.py
...
def build_edge_sets_for_layer(data, layer_name: str, train_ratio=0.7, val_ratio=0.15, seed=42, neg_mode = 'uniform'):
    ...
    if neg_mode == "uniform":
        train_neg = sample_negative_edges(num_nodes, train_pos.size(1), existing, seed=seed + 1)
        val_neg = sample_negative_edges(num_nodes, val_pos.size(1), existing, seed=seed + 2)
        test_neg = sample_negative_edges(num_nodes, test_pos.size(1), existing, seed=seed + 3)
    elif neg_mode == "hard_region":
        train_neg = sample_negative_edges_hard_region(data, train_pos.size(1), existing, seed=seed + 1)
        val_neg = sample_negative_edges_hard_region(data, val_pos.size(1), existing, seed=seed + 2)
        test_neg = sample_negative_edges_hard_region(data, test_pos.size(1), existing, seed=seed + 3)
    else:
